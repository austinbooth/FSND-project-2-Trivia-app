...
def create_app(test_config=None):
    ...
    def delete_question(id):
        try:
            ...
            question.delete()
            selection = Question.query.order_by(Question.id).all()
            current_questions = current_questions = paginate_questions(
                request, selection)
    ...
    end of the last page of the questions list in the
    "List" tab.
    '''
    ...
    def play_quiz():
        ...
        quiz_cat_id = body.get('quiz_category')['id']

        if (quiz_cat_id == 0):
            selection = Question.query.order_by(Question.id).all()
        else:
            selection = Question.query.filter(
                Question.category == quiz_cat_id).order_by(
                Question.id)
        ...
        choice_id = random.choice(question_ids)
        chosen_question = Question.query.get(choice_id)

        send_q = chosen_question.format()
